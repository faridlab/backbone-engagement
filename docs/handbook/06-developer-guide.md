...
metaphor migration generate Invoice engagement
metaphor migration run
...
(`engagement` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed engagement          # run Rust seeders
metaphor migration generate-seeds engagement  # emit SQL seed files
```
